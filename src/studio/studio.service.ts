...
export class StudioService {
  ...
  // v2 — сторона кадра поднята (неподвижный 512→1024, кадр турнтейбла
  // 256→512): прежние размеры подбирались под мелкую карточку витрины и
  // расползались в кашу в окне крупного просмотра и на публичной странице.
  //
  // Ровно ради этого версия и лежит в пути. Поднятая версия — это другой
  // адрес, поэтому старые файлы просто перестают запрашиваться: ни сбрасывать
  // кеши, ни удалять что-либо не нужно. Ассеты без превью v2 витрина увидит
  // как ненарисованные и переснимет их сама.
  //
  // Превью v1 остаются в бакете мёртвым грузом. Их можно вычистить запросом
  // по префиксу thumbnails/%/v1/, но это уборка, а не часть правки.
  private readonly PREVIEW_VERSION = 2;
