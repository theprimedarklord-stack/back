...
export class PaddleService {
  ...
  async handleEvent(event: any) {
    ...
    // Identify organization ID from customData
  ...
  async getInvoices(customerId: string) {
    // Paddle has a Transactions API, not an Invoices API
    ...
    for await (const t of transactions) {
      if (t.status === 'completed' || t.status === 'paid') {
        ...
        invoices.push({
          ...
          status: 'Оплачено',
          pdfUrl: null, // Paddle doesn't have a direct invoice_pdf URL, typically you retrieve it via Customer Portal
        });
      }
    }
    return invoices;
